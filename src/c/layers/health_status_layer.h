...
// font). Mirrors weather_status_layer; the window pushes it from spec.status_tier
// (view_spec_from_state() in windows/layout.c). Must be set before the first layout/paint.
...
// false; set before the first layout/paint. See health_status_layout() and the ViewSpec tier
// (spec.status_tier / view_spec_from_state() in windows/layout.c).
