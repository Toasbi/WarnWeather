...
// A subheader carrying a `hint` (instead of an `intro`) renders as a ROW: its hosted
// switch's own toggle row, with the group title as the label and the hint under it —
// so a card of ordinary rows (the Nighttime card) keeps the row rhythm instead of the
// heading's standoffs. Being a row, it takes the look-ahead's join class like any row:
// tight (nb) when the first row its switch reveals joins it, loose (nbl) when the next
// thing rendered is another group header, and it still owns the line above it (.grp).
test('renderBody: a subheader with a hint renders as its switch\'s row and joins like one', () => {
  const SCH = { appName: 'X', versionLabel: 'v0', tabs: [ { id: 't', label: 'T', sections: [ { title: 'S', items: [
    { type: 'toggle', messageKey: 'lead', label: 'Lead', defaultValue: false },
    { type: 'subheader', text: 'Grp', toggleKey: 'grpOn', hint: 'Grp explains itself.' },
    { type: 'toggle', messageKey: 'grpOn', label: 'Grp switch', defaultValue: false },
    { type: 'toggle', messageKey: 'dep', label: 'Dep', defaultValue: false, joinPrevious: true,
      showWhen: { key: 'grpOn', eq: true } },
    { type: 'subheader', text: 'Next', toggleKey: 'nextOn', hint: 'Next explains itself.' },
    { type: 'toggle', messageKey: 'nextOn', label: 'Next', defaultValue: false }
  ] } ] } ] };
  const render = (over) => {
    const S = Object.assign(E.hydrate(SCH, {}), over || {});
    return E.renderBody(SCH, 't', { S: S, ENV: { color: true }, USERDATA: {}, openColor: null,
  ...
  const off = render({ grpOn: false });
  assert.equal(off.indexOf('subhdr'), -1, 'no heading bar');
  assert.equal(off.indexOf('class="intro"'), -1, 'no intro block: the copy is the row\'s hint');
  assert.ok(off.indexOf('<div class="row grp nbl"><div class="lft"><div class="lbl">Grp</div>'
    + '<div class="hint">Grp explains itself.</div></div><div class="rgt">'
    + '<button class="sw" data-k="grpOn" data-toggle="1" aria-label="Grp switch"><i></i></button></div></div>') !== -1,
    'the header IS a toggle row: label + hint on the left, the hosted switch on the right, '
    + 'still named by the hosted toggle\'s own label (the visible title names the group)');
  assert.equal(off.split('data-k="grpOn"').length - 1, 1, 'the switch renders once, on the header row');
  assert.equal(rowClass(off, 'lead'), 'row nbl', 'the row above hands its line to the header, loosely');
  assert.equal(rowClass(off, 'grpOn'), 'row grp nbl',
    'a collapsed group\'s header hands its line to the next header, loosely');
  assert.equal(rowClass(off, 'nextOn'), 'row grp', 'the card\'s last row joins nothing');

  const on = render({ grpOn: true });
  assert.equal(rowClass(on, 'grpOn'), 'row grp nb',
    'the first row the switch reveals joins the header tight, as it would join any row');
  assert.equal(rowClass(on, 'dep'), 'row nbl', 'the group\'s last row hands its line to the next header');
});

test('renderBody: a row-shaped subheader joins the same way whatever its hint\'s length', () => {
  // A short hint lays the header out as a centred two-column row, a long one as .wrap
  // (label on top, switch beside the hint) — renderRow's own heuristic. Either way it is
  // the same row to the look-ahead, so a copy edit cannot bring the card's steps back.
  const joins = (hint) => {
    const SCH = { appName: 'X', versionLabel: 'v0', tabs: [ { id: 't', label: 'T', sections: [ { title: 'S', items: [
      { type: 'subheader', text: 'Grp', toggleKey: 'grpOn', hint: hint },
      { type: 'toggle', messageKey: 'grpOn', label: 'Grp', defaultValue: true },
      { type: 'toggle', messageKey: 'dep', label: 'Dep', defaultValue: false, joinPrevious: true },
      { type: 'subheader', text: 'Next', toggleKey: 'nextOn', hint: hint },
      { type: 'toggle', messageKey: 'nextOn', label: 'Next', defaultValue: false }
    ] } ] } ] };
    const S = E.hydrate(SCH, {});
    const html = E.renderBody(SCH, 't', { S: S, ENV: { color: true }, USERDATA: {}, openColor: null,
      collapsed: {}, evalCtx: Object.assign({}, S, { env: { color: true } }) });
    return (html.match(/class="row[^"]*"/g) || []).map((c) => c.replace(/ wrap\b/, ''));
  };
  const short = 'Pause updates overnight.';
  const long = 'Stop sending updates to your watch between the hours below to save battery.';
  assert.ok(short.length <= 64 && long.length > 64, 'one each side of the wrap threshold');
  assert.deepEqual(joins(short), joins(long));
  assert.deepEqual(joins(short), ['class="row grp nb"', 'class="row nbl"', 'class="row grp"']);
});

test('renderBody: a subheader with an intro stays a heading and ignores the join look-ahead', () => {
  // The threshold sheets' shape: heading bar with the switch, the copy as a full-width
  // intro. It paints no bottom divider, so the look-ahead has nothing to class on it —
  // the row below keeps its own padding whatever it declares.
  const SCH = { appName: 'X', versionLabel: 'v0', tabs: [ { id: 't', label: 'T', sections: [ { title: 'S', items: [
    { type: 'subheader', text: 'Grp', toggleKey: 'grpOn', intro: 'About the group.' },
    { type: 'toggle', messageKey: 'grpOn', label: 'Highlight', defaultValue: true },
    { type: 'toggle', messageKey: 'dep', label: 'Dep', defaultValue: false, joinPrevious: true }
  ] } ] } ] };
  const S = E.hydrate(SCH, {});
  const html = E.renderBody(SCH, 't', { S: S, ENV: { color: true }, USERDATA: {}, openColor: null,
    collapsed: {}, evalCtx: Object.assign({}, S, { env: { color: true } }) });
  assert.ok(html.indexOf('<div class="subhdr grp"><span>Grp</span><button class="sw on" data-k="grpOn"'
    + ' data-toggle="1" aria-label="Highlight"><i></i></button></div><div class="intro">About the group.</div>') !== -1,
    'heading + intro, unchanged');
  assert.equal(html.indexOf('row grp'), -1, 'no row-shaped header');
});
