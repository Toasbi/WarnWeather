...
// B&W platforms get a single polarity-foreground stop (white in dark, black in light);
// the watch adds a polarity-background 1px border.
 ...
 * foreground (theme_fg() on the watch: white in dark polarity, black in light
 * polarity — see palette.c fill_defaults / health_graph_layer.c) and ignore
 * colorMode; the watch pairs it with a theme_bg() 1px border (chart.c's
 * bar-separation halo). On effectively-color displays, 'white' collapses to a
 * single stop — white in dark polarity, GColorDarkGray in light polarity (a
 * pure white bar reads too flat against a white background) — anything else
...
function buildPalette(platform, colorMode, theme) {
    ...
    if (!configUi.isColorPlatform(platform) || isBwTheme(theme)) {
        return { from: [0], rgb: [isLightPolarity(theme) ? COLORS.GColorBlack : COLORS.GColorWhite] };
    }
    if (colorMode === 'white') {
