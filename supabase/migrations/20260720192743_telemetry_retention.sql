...
begin
  -- 1. Rebuild telemetry_dau for every complete UTC day still in raw.
  delete from public.telemetry_dau
   where activity_date in (
     select distinct (received_at at time zone 'UTC')::date
      ...
      where (received_at at time zone 'UTC')::date < v_today
   );
  ...
  join lateral (
    ...
    where coalesce(t.watch_token_hash, t.account_token_hash) = w.watch_key
      and (t.received_at at time zone 'UTC')::date < v_today
    order by t.received_at desc
    limit 1
  ) l on true
  ...
  -- 3. Rebuild telemetry_errors for the same complete days still in raw.
  ...
  from public.telemetry_weather_fetch
  where not success and (received_at at time zone 'UTC')::date < v_today;
