...
generators in scripts/):
  ...
  src/c/layers/clock_glyphs_metrics.h        each glyph's strip column, width, left
      bearing and advance, plus the union ink height — the numbers clock_glyphs.c
      lays the string out with.
...
def write_header(results):
    ...
    for i, platform in enumerate(PLATFORMS):
        ...
        for name, _stem, _ttf, _sizes in FACES:
            ...
            out.append('#define CLOCK_GLYPHS_%s_INK_H %d  // %s px' % (name, r['ink_h'], r['size']))
            out.extend(c_table(name, r['glyphs'], r['xs']))
    out.extend([
        '#else',
    ...
    path = os.path.join(ROOT, 'src', 'c', 'layers', 'clock_glyphs_metrics.h')
...
def main():
    ...
    print('wrote', os.path.relpath(write_header(results), ROOT))
    if args.preview:
