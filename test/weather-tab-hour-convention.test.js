...
test('startHourFields pairs each hour with the stamp an hour later, by time', () => {
  ...
  assert.deepEqual(from, [1, 2, -1, 4, -1], 'the index each row took its values from');
  assert.deepEqual(hourly.rain, [2, 3, null, 6, null],
    'row 02:00 has no 03:00 partner, and an index shift would have handed it 04:00\'s');
  assert.deepEqual(hourly.measured, [false, true, false, true, false], 'each key takes its own fill');
  assert.deepEqual(hourly.temp, [10, 11, 12, 14, 15], 'series it is not given stay on their stamp');
});
...
test('an Open-Meteo day tile totals the same bars it sits over', () => {
  ...
  const BASE = Math.floor(DAY0 / 1000);
  const off = -new Date(DAY0).getTimezoneOffset() * 60;
  const hourly = { time: [], temperature_2m: [], precipitation: [], precipitation_probability: [] };
  for (let i = -24; i < 72; i += 1) {
    hourly.time.push(BASE + i * 3600);
    hourly.temperature_2m.push(12);
    hourly.precipitation.push(i === 0 ? 5 : (i === 24 ? 3 : 0));
    hourly.precipitation_probability.push(i === 0 ? 90 : (i === 24 ? 40 : 10));
  }
  const daily = { time: [BASE, BASE + 86400, BASE + 2 * 86400],
    temperature_2m_max: [15, 15, 15], temperature_2m_min: [9, 9, 9],
    precipitation_sum: [5, 3, 0], precipitation_probability_max: [90, 40, 10],
  ...
  assert.equal(out.daily[1].rainMm, 0, 'tomorrow keeps nothing of today\'s');
