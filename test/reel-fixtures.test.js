...
test('emery manifest: intro + all three captioned chapters in order', () => {
  ...
  assert.deepStrictEqual(intro, ['scene_1.png', 'scene_7.png', 'scene_8.png', 'scene_9.png',
    'scene_6.png'], 'the showcase scenes, in their order');
...
test('aplite manifest: no themes card/segments, no radar/health graph or health status', () => {
  ...
  assert.deepStrictEqual(m.filter((s) => s.group === 'intro').map((s) => s.frame),
    ['scene_1.png', 'scene_6.png'], 'no colour-only intro scenes');
